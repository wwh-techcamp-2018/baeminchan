function addBestMenuUi(result) {
  result.json().then(menus => {
    menus.forEach((menu, i) => {
      $(".tab-btn-box").insertAdjacentHTML(
        "beforeend",
        makeHTMLMenuBtn(menu, i)
      );
      $(".tab-content-group-box").insertAdjacentHTML(
        "beforeend",
        makeHtmlMenuTab(menu.products)
      );
    });
    onRandomTab();
  });
...
function addBestMenuEventHandlers() {
